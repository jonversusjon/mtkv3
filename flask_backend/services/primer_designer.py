...
class PrimerDesigner:
    ...
    def design_mutation_primers(
            ...
            with logger.timer_context(f"Process Mutation Set {idx + 1}"):
                ...
                with logger.timer_context("Coordinate Selection"):
                    if max_results == 0:
                        ...
                        coords_to_process = [
                            valid_coords[i].tolist() for i in selected_indices
                        ]
                        logger.log_step(
                ...
                if mut_set_primer_pairs:
                    primer_set = MutationPrimerSet(
                        mut_primer_pairs=mut_set_primer_pairs
                    )
        ...
        send_update(
            message=f"{len(all_primers)} mutation primer sets designed successfully",
            prog=100,
            primers=all_primers,
        )
        return all_primers
        ...
        for i, rs_key in enumerate(mut_rs_keys):
            with logger.timer_context(f"Process Restriction Site {rs_key}"):
                ...
                with logger.timer_context("Design Reverse Primer"):
                    ...
                    while (
                        self.utils.calculate_tm(r_anneal) < tm_threshold
                        and r_seq_length < self.max_binding_length
                    ):
                        r_seq_length += 1
                ...
                with logger.timer_context("Construct Primer Pair"):
                    ...
                    r_primer = Primer(
                        name=(primer_name + "_reverse")
                        if primer_name
                        else f"primer_{i}_reverse",
