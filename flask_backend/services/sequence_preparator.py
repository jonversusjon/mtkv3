...
class SequencePreparator:
    ...
    def preprocess_sequence(
        self, sequence: Union[str, Seq], matk_part_left: str, send_update: Callable
        ...
        if (
            matk_part_left in {"3", "3a"}
            and len(cleaned_sequence) >= 3
            and cleaned_sequence[:3] == "ATG"
        ):
        ...
        if remainder != 0:
            if trim_start_codon:
                ...
                send_update(
                    message=f"Adjusting frame by trimming {remainder} bases from the end",
                    prog=80,
                )
            elif trim_stop_codon:
                cleaned_sequence = cleaned_sequence[remainder:]
                send_update(
                    message=f"Adjusting frame by trimming {remainder} bases from the beginning",
                    prog=80,
                )
            else:
                send_update(
                    message="Warning: Sequence not in frame and no codons to trim",
                    prog=80,
                )
        else:
            send_update(
                message="Sequence already in frame, no adjustment needed", prog=80
            )
        ...
        if not in_frame:
            if trim_start_codon and trim_stop_codon:
                message = (
                    "Provided sequence does not appear to be in frame, using provided start codon "
                ...
                message = (
                    "Provided sequence does not appear to be in frame, using provided stop codon "
                ...
                send_update(
                    message="Sequence not in frame and cannot be corrected",
                    prog=100,
                ...
                return sequence, message, valid_sequence
        else:
            if trim_start_codon and trim_stop_codon:
        ...
        send_update(
            message=f"Preprocessing complete: {message}",
            prog=100,
            notification_count=notification_count,
            notification_type=notification_type,
            callout=message,
        ...
        return Seq(str(cleaned_sequence)), message, valid_sequence
