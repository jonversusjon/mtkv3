...
class Logger:
    def __init__(
        ...
        # Use single file logging mode if requested
        self.single_file_logging = single_file_logging
        self._setup_handlers(enable_file_logging, log_dir, custom_format)

        # If using single file mode, function and timer logs go to the same file
        if not single_file_logging:
            ...
            function_handler = logging.FileHandler(
                os.path.join(log_dir, f"function_calls_{timestamp}.log"),
                encoding="utf-8",
            )
            ...
            self.timer_csv_filename = os.path.join(
                log_dir, f"timer_logs_{timestamp}.csv"
            )
            os.makedirs(log_dir, exist_ok=True)
            with open(self.timer_csv_filename, "w", newline="") as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(
                    ["Operation", "Start Time", "End Time", "Elapsed Seconds"]
                )
        else:
            # For single file logging, use the main logger for function logs
            self.function_logger = self.logger
            self.timer_csv_filename = None
    ...
    def _setup_handlers(self, enable_file_logging, log_dir, custom_format):
        ...
        if enable_file_logging:
            ...
            # Use a different naming scheme for single file logging mode
            if self.single_file_logging:
                log_filename = f"{log_dir}/mtk_application_{timestamp}.log"
            else:
                log_filename = f"{log_dir}/app_logger_{timestamp}.log"
    ...
    @staticmethod
    @contextmanager
    def logged_timer_context(
        operation: str, logger, level: str = "info", debug: bool = False
    ):
        """
    ...
    def debug_context(self, operation: str):
        ...
        """
        with Logger.logged_timer_context(
            operation, self.logger, level="debug", debug=True
        ):
            yield
    ...
    def timer_context(self, operation: str, level: str = "info"):
        ...
        try:
            ...
            # Only write to CSV if not in single file logging mode
            if not self.single_file_logging and self.timer_csv_filename:
                try:
                    ...
                    self.logger.error(
                        f"Failed to write CSV log for operation {operation}: {csv_e}"
                    )
    ...
    def log_function(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            self.function_logger.info(
                f"| COMPLETED: {func_name} in {elapsed:.4f} seconds with result={result}"
            )
            self.function_logger.info(f"+{'-' * 60}")
            return result
    ...
    def log_step(self, step_name, message, data=None, level=logging.INFO):
        ...
        try:
            if isinstance(data, BaseModel):
                ...
                isinstance(v, BaseModel) for v in data.values()
            ):
                data_str = json.dumps(
                    {k: v.model_dump() for k, v in data.items()}, indent=2
                )
            else:
        ...
        if data_str:
            log_message = f"STEP: {step_name} - {message}\nData: {data_str}"
        else:
            log_message = f"STEP: {step_name} - {message}"
        ...
        # Also print a debug message to confirm the log was recorded
        print(f"[Logger] Step recorded: {step_name} - {message}")
    ...
    def log_execution(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            elapsed = end - start
            logging.info(
                f"| COMPLETED: {func.__name__} in {elapsed:.4f} seconds with result={result}"
            )
            return result
...
logger = Logger(log_dir="flask_backend/log_utils/logs", single_file_logging=True)
